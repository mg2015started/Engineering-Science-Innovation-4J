...
class BP_network:
    def __init__(self):
        ...
        # parameters (w, t)
        self.ih_u = []  # weight for each link
        self.ho_u = []
        self.ih_v = []  # weight for each link
        self.ho_v = []
    ...
    def CreateNN(self, ni, nh, no, actfun):
        ...
        self.ih_u = np.zeros([self.i_n, self.h_n])
        self.ho_u = np.zeros([self.h_n, self.o_n])
        self.ih_v = np.zeros([self.i_n, self.h_n])
        self.ho_v = np.zeros([self.h_n, self.o_n])
        for i in range(self.i_n):
            for h in range(self.h_n):
                self.ih_u[i][h] = rand(0, 1)
                self.ih_v[i][h] = rand(0, 1)
        for h in range(self.h_n):
            for j in range(self.o_n):
                self.ho_u[h][j] = rand(0, 1)
                self.ho_v[h][j] = rand(0, 1)
    ...
    def Pred(self, x):
        ...
        for h in range(self.h_n):
            total = 0.0
            for i in range(self.i_n):
                total += self.i_v[i]**2 * self.ih_u[i][h]+self.i_v[i]* self.ih_v[i][h]
        ...
        for j in range(self.o_n):
            total = 0.0
            for h in range(self.h_n):
                total += self.h_v[h]**2 * self.ho_u[h][j]+self.h_v[h] * self.ho_v[h][j]
    ...
    def BackPropagate(self, x, y, lr):
        ...
        for h in range(self.h_n):
            for j in range(self.o_n):
                h_grid[h] += (2*self.ho_u[h][j]*self.h_v[h]+self.ho_v[h][j]) * o_grid[j]
            h_grid[h] = h_grid[h] * self.afd(self.h_v[h])

            # updating the parameter
        for h in range(self.h_n):
            for j in range(self.o_n):
                self.ho_u[h][j] += lr * o_grid[j] * (self.h_v[h]**2)
                self.ho_v[h][j] += lr * o_grid[j] * self.h_v[h]

        for i in range(self.i_n):
            for h in range(self.h_n):
                self.ih_u[i][h] += lr * h_grid[h] * (self.i_v[i]**2)
                self.ih_v[i][h] += lr * h_grid[h] * self.i_v[i]
